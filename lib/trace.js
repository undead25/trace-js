...
var Report = function () {
    ...
    createClass(Report, [{
        ...
        key: 'handleException',
        value: function handleException(type, message, fileName, lineNumber, columnNumber, frames) {
            ...
            if (frames && frames.length) {
                ...
                stacktrace = {
                    frames: [{ fileName: fileName, lineNumber: lineNumber, columnNumber: columnNumber }]
                };
            }
        ...
        value: function isSameStacktrace(stacktrace1, stacktrace2) {
            ...
            stacktrace1.forEach(function (item, index) {
                if (item.fileName !== stacktrace2[index].fileName || item.columnNumber !== stacktrace2[index].columnNumber || item.lineNumber !== stacktrace2[index].lineNumber || item.func !== stacktrace2[index].func) return false;
            });
            return true;
        }
    }]);
...
var Exception = function () {
    ...
    createClass(Exception, [{
        ...
        value: function handleWindowOnError() {
            var self = this;
            var stackInfo = null;
            window.onerror = function (message, fileName, lineNumber, columnNumber, error) {
                if (error) {
                    ...
                    var frame = { fileName: fileName, lineNumber: lineNumber, columnNumber: columnNumber, func: '?' };
                    var type = void 0,
                        msg = message;
                    if (Object.prototype.toString.call(message) === '[object String]') {
                        // example: ['Uncaught TypeError: blah blah blah', 'TypeError' 'blah blah blah']
                        var msgGroup = message.match(self.ERROR_TYPES);
                        if (msgGroup) {
                            type = msgGroup[1];
                            msg = msgGroup[2];
                        }
            ...
            for (var i = 0, j = lines.length; i < j; ++i) {
                if (parts = chrome.exec(lines[i])) {
                    ...
                    if (isEval && (submatch = geckoEval.exec(parts[3]))) {
                        ...
                        frames[0].columnNumber = error.columnNumber + 1;
                    }
                    element = {
                        fileName: parts[3],
                        func: parts[1] || '?',
        ...
        value: function analyzeStackFromCaller(error, depth) {
            ...
            for (var curr = this.analyzeStackFromCaller.caller; curr && !recursion; curr = curr.caller) {
                ...
                typeof frame.func === 'undefined' && (frame.func = parts.input.substring(0, parts.indexOf('{')));
...
var Trace = function () {
    ...
    createClass(Trace, [{
        ...
        value: function captureMessage(message) {
            ...
            var stackInfo = this.analyzeErrorStack(exception);
            var frames = this.report.prepareFrames(stackInfo);
